...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class AudioPlay : FsmStateAction
	{
		...
		[Tooltip("Make the action finish by default as soon the audio play is called. ")]
		public FsmBool finishActionImmediatelly;

		[Tooltip("Ignore this action if the audio source is already playing.")]
		public FsmBool ignoreIfAlreadyPlaying;
		...
		public override void Reset()
		{
			...
			oneShotClip = null;
		    finishedEvent = null;
			finishActionImmediatelly = new FsmBool{ Value = false };
			ignoreIfAlreadyPlaying = new FsmBool { Value = false };
		}
		...
		public override void OnEnter()
		{
			...
			if (go != null)
			{
				...
				if (audio != null)
				{
					...
					if (audioClip == null)
					{
						if ( !ignoreIfAlreadyPlaying.Value || ignoreIfAlreadyPlaying.Value && !audio.isPlaying ) {
							audio.Play();
						}
		...
		public override void OnUpdate ()
		{
			...
			{
				if (!audio.isPlaying || finishActionImmediatelly.Value)
				{
