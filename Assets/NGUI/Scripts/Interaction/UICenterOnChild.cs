...
public class UICenterOnChild : MonoBehaviour
{
	...
	public UIDraggablePanel CachedDragPanel()
	{
		if (mDrag == null)
		{
	...
	protected void ScrollToClosestChild(Transform closestChildOverride, float overrideSpringStrength)
	{
		
	}
	
	protected SpringPanel ScrollToClosestChild(Transform closestChildOverride = null)
	{
		...
		// If the closest child wasn't explicitly given to this method then we have to determine it automatically and center on it.
		if (closestChildOverride == null) 
		{
		...
		if (closestChildOverride != null)
		{
			mCenteredObject = closestChildOverride.gameObject;

			// Figure out the difference between the chosen child and the panel's center in local coordinates
			Vector3 cp = dt.InverseTransformPoint(closestChildOverride.position);
			...
			springPanel = SpringPanel.Begin(mDrag.gameObject, dt.localPosition - offset, springStrength);
	...
	{
		if ( CachedDragPanel() == null )
		{
			return;
		}
	
		ScrollToClosestChild(null);
	}
	
	public void Recenter(GameObject targetChild)
	{
		if ( CachedDragPanel() == null )
		{
			return;
		...
		ScrollToClosestChild(targetChild.transform);
	}	
}