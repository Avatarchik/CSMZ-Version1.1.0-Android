...
{
	public struct OverrideFontInfo
	{
		public Color color;
		public Color effectColor;
		public bool canOverrideColor;
		public bool canOverrideEffectColor;
				
		public OverrideFontInfo(Color _color)
		{
			color = _color;
			effectColor = Color.white;
			canOverrideColor = true;
			canOverrideEffectColor = false;
		}
		
		public OverrideFontInfo(bool _canOverrideColor, Color _color,  bool _canOverrideEffectColor, Color _effectColor)
		{
			color = _color;
			effectColor = _effectColor;
			canOverrideColor = _canOverrideColor;
			canOverrideEffectColor = _canOverrideEffectColor;			
		}		
	}
	...
	public void ApplyFontOverrideInfo(OverrideFontInfo overrideFontInfo)
	{
		if (overrideFontInfo.canOverrideColor) {
			color = new Color(overrideFontInfo.color.r, overrideFontInfo.color.g, overrideFontInfo.color.b, color.a);
		}
		
		if (overrideFontInfo.canOverrideEffectColor) {
			effectStyle = Effect.Outline;
			effectColor = overrideFontInfo.effectColor;
		}
	}
	...
	protected override void OnStart ()
	{
		...
		//TODO: hack to support different fonts for our frozen purposes different languages for all UILabels
		if (Application.isPlaying && GetComponent<IgnoreFontTranslations>() == null)
		{
			string langString = "";
			LanguageCode currentLanguage = Language.CurrentLanguage();
			
			if (currentLanguage == LanguageCode.RU) {
				langString = "Russian";
				transform.localScale = transform.localScale * 0.9f;
			}else if (currentLanguage == LanguageCode.JA) {
				langString = "Japanese";
				
				if (font != null && font.name != "Edwardian Medium LET 56" && font.name != "Edwardian Medium LET 32") 
				{
				...
				{
					transform.localScale = transform.localScale * 0.9f;
					//transform.localPosition = transform.localPosition + new Vector3(0f, -1f, 0f);
				}
			}
			else if (currentLanguage == LanguageCode.KO) {
				if (font != null && font.name == "TitleFont") {
					langString = "Korean";
					transform.localScale = transform.localScale * 0.7f;
					transform.localPosition = transform.localPosition + new Vector3(0f, -8f, 0f);
				}
			}
			...
			if (langString != "")
			{
				if (font != null)
				{
					string previousFontName = font.name;
					
					// Apply override font corresponding to english font size
					if (transform.localScale.x >= 36f) {
#if UNITY_ANDROID
						font = (Resources.Load("Fonts/" + langString + "_Big_Android") as GameObject).GetComponent<UIFont>();
#else
						font = (Resources.Load("Fonts/" + langString + "_Big") as GameObject).GetComponent<UIFont>();
#endif					
					}
					else {
#if UNITY_ANDROID
						font = (Resources.Load("Fonts/" + langString + "_Small_Android") as GameObject).GetComponent<UIFont>();
#else
						font = (Resources.Load("Fonts/" + langString + "_Small") as GameObject).GetComponent<UIFont>();
#endif					
					}
				}
			}
