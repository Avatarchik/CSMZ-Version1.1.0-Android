...
public class UIAtlas : MonoBehaviour
{
	...
	public UIAtlas mReplacement {
		get {
			if(atlas == null) {
				if (!name.EndsWith("SD") && !name.EndsWith("HD")) {

					string path = "Atlases/"+name;

					path += QualitySetter.useHDAtlas ? "HD" : "SD";
					
					GameObject gObjAtlas = Resources.Load(path, typeof(GameObject)) as GameObject;
					
					if (gObjAtlas != null) {
						atlas = gObjAtlas.GetComponent<UIAtlas>();
						if(atlas == null) {
	//						Debug.Log("Can not find replacement atlas on "+ path);
						}
					}else {
	//					Debug.Log("Resource " + path + " not found!");
					}
				}
			}
